...
freeproc(struct proc *p)
{
  ...
  if (p -> kernel_pagetable)
    free_kernelpgtbl(p -> kernel_pagetable, 0);
  p->pagetable = 0;
  p->sz = 0;
  p->pid = 0;
  p->parent = 0;
...
userinit(void)
{
  ...
  p->sz = PGSIZE;
  u2kmerge(p->pagetable, p->kernel_pagetable, 0, p->sz);
...
growproc(int n)
{
  ...
  u2kmerge(p->pagetable, p->kernel_pagetable, p->sz, sz);
  p->sz = sz;
  return 0;
}
...
fork(void)
{
  ...
  u2kmerge(np->pagetable, np->kernel_pagetable, 0, np->sz);
